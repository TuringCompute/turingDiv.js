...
class DivEle{
    ...
    isDupEvent(eventObj){
        ...
        this.cleanEventQueue(currentStamp)
        if(!eventObj[EventSrc.Key.uId]){
            //if no event Uid in event object, decide it is duplicate event
            return false
        }
        let eId = eventObj[EventSrc.Key.uId]
        let isDup=false
        if(this.eventQueue[eId]){
            isDup = true
        }
        this.eventQueue[eId] = currentStamp
        return isDup
    }

    cleanEventQueue(currentStamp){
        for(let uid in this.eventQueue){
            if((currentStamp - this.eventQueue[uid]) > this.eventDupLimit){
                delete this.eventQueue[uid]
            }
        }
    }
