...
class DivEle{
    ...
    isDupEvent(eventObj){
        ...
        let eIdx = this.eventQueue.indexOf(eId)
        if(eIdx > -1){
            this.eventQueue.splice(eIdx, 1)
        }
        this.eventQueue.push(eId)
        if(this.eventQueue.length > this.eventQueueSize){
            this.eventQueue.splice(0, (this.eventQueue.length - this.eventQueueSize))
        }
        return eIdx > -1
    }
