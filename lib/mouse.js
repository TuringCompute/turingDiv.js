...
class MouseState{
    ...
    fireEvent(eventType, e){
        if(this.target){
            let eventData = {}
            if(this.targetTags){
                eventData = this.targetTags
            }
            let eventObj = EventSrc.new(eventType, MouseState.mouse, eventData)
            eventObj[EventSrc.Key.rawEvent] = e
            eventObj[EventSrc.Key.src] = this.target.id
    ...
    registerTarget(target, targetTag){
